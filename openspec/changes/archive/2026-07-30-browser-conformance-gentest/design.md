...
5. **Determinism controls**: fixed viewport, `deviceScaleFactor: 1`, headless. Byte-stable XML output (stable attribute order, fixed float formatting matching the Rust writer).
   **Revised during implementation — scrollbars:** classic space-taking scrollbars can NOT be enforced in current Chrome: headless Chrome reports 0-width scrollbars on macOS and Linux regardless of `OverlayScrollbar`/`::-webkit-scrollbar` overrides (verified empirically on Chrome 151 and Chrome 120, host and container). Taffy's own gentest depended on the generating desktop's system settings (their error message instructs changing macOS System Settings). Decision: the canonical environment is plain pinned **headless Chrome with 0-width scrollbars** — the only configuration reproducible with one command on any machine. Scrollbar width is UA-defined, not spec-defined, so 0 is as conformant as 15; the ~240 scroll fixtures regenerate with `scrollbar-width="0"` and matching geometry, and the engine's scrollbar-gutter logic (an embedder API knob) keeps coverage through dedicated unit tests instead of browser fixtures. The gentest canary now *records* the measured scrollbar width into provenance rather than asserting non-zero.
