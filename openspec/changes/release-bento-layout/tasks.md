...
- [ ] 3.1 Commit tasks 1–2 on `main`
- [ ] 3.2 Create the public repo: `gh repo create julia-script/bento-layout --public --source . --remote origin --description "Flexbox, grid, and block layout in plain TypeScript — no WASM, no dependencies"`
- [ ] 3.3 Push `main` and confirm `.github/workflows/release.yml` is present on the default branch — Trusted Publishing validates against the workflow in the repo
- [ ] 3.4 Confirm the pushed tree contains no `node_modules/`, `dist/`, `.next/`, or `.source/`
- [ ] 3.5 Set the repo's homepage to `https://bento.jlort.com` and add topics
...
- [ ] 6.1 Create the project from the GitHub repo, framework preset Next.js, on team `juliascripts-projects`
- [ ] 6.2 Set Root Directory to the **repo root** — not `docs/`, which prunes `../src` from the build context and breaks every library import
- [ ] 6.3 Set Install Command `pnpm --dir docs install --frozen-lockfile`, Build Command `pnpm --dir docs build`, Output Directory `docs/.next`
- [ ] 6.4 Trigger a deployment and confirm it builds — expect 15 prerendered routes, matching the verified local build
- [ ] 6.5 On the `*.vercel.app` URL, confirm `/`, `/playground`, and a docs page all return 200, and that a playground edit computes a layout
- [ ] 6.6 Confirm an unknown route returns 404 rather than a server error

## 7. [USER] DNS

- [ ] 7.1 **[USER]** In Cloudflare DNS for `jlort.com`, add `CNAME bento → cname.vercel-dns.com` with proxy **disabled (grey cloud)**. Orange-cloud breaks Vercel's certificate issuance and stacks two CDNs. This record overrides the existing wildcard that currently returns 503. Assistant pauses here and confirms before proceeding to task 8.
- [ ] 7.2 Add `bento.jlort.com` as a domain on the Vercel project and confirm it verifies
- [ ] 7.3 Confirm `dig bento.jlort.com` returns Vercel's address, not the previous Cloudflare wildcard IPs
- [ ] 7.4 Confirm `https://bento.jlort.com` returns 200 with a valid certificate covering the hostname

## 8. Docs metadata

- [ ] 8.1 Add a `metadata` export to `docs/app/layout.tsx` with `metadataBase: new URL('https://bento.jlort.com')` — without it Next emits relative social image URLs that scrapers reject, silently nullifying the rest of this section
- [ ] 8.2 Add a title `template` (`'%s | bento-layout'`) with a `default`, plus root `description`, `openGraph`, and `twitter` (`summary_large_image`)
- [ ] 8.3 Verify the docs pages now compose titles correctly — they set bare titles like "Getting Started" today and must render with the project name appended
- [ ] 8.4 Add `docs/app/opengraph-image.tsx` — 1200×630 `ImageResponse` hero card using the bento tokens (washi `hsl(40,45%,97%)`, coral `hsl(13,72%,62%)`, the mark from `app/icon.svg`)
- [ ] 8.5 Wire per-page doc cards via fumadocs `createMetadataImage` so `/docs/reference/api` previews with its own title; fall back to a system serif if fetching the display font at build time proves brittle
- [ ] 8.6 Add `docs/app/apple-icon.png` at 180×180 — iOS ignores SVG icons
- [ ] 8.7 Add a `viewport` export with light/dark `themeColor` drawn from the paper tokens
- [ ] 8.8 Add `docs/app/robots.ts` permitting indexing and referencing the sitemap's absolute URL
- [ ] 8.9 Add `docs/app/sitemap.ts` derived from `source.getPages()`, appending `/` and `/playground`; confirm a newly added `.mdx` appears without further edits

## 9. Verify the release

- [ ] 9.1 Build locally and confirm the OG images render at 1200×630 with legible text, not a blank or fallback card
