...
describe('scrollbar gutter reservation', () => {
  it('flex: scroll container reserves gutters for its content', () => {
    ...
    const child = createNode({ style: { size: { width: { percent: 1 }, height: { percent: 1 } } } });
    const root = createNode({
      style: { size: { width: 100, height: 100 }, overflow: SCROLL, scrollbarWidth: 15 },
      children: [child],
    });
  ...
  it('flex: vertical scrollbar consumes inline space in a row', () => {
    ...
    const root = createNode({
      style: { size: { width: 100, height: 50 }, overflow: { x: 'visible', y: 'scroll' }, scrollbarWidth: 15 },
      children: [a],
    });
    computeLayout(root, { width: 'max-content', height: 'max-content' });
  ...
  it('block: scroll container reserves gutters', () => {
    const child = createNode({ style: { display: 'block', size: { width: 'auto', height: 20 } } });
    const root = createNode({
      style: { display: 'block', size: { width: 100, height: 100 }, overflow: SCROLL, scrollbarWidth: 15 },
      children: [child],
    });
    computeLayout(root, { width: 'max-content', height: 'max-content' });
